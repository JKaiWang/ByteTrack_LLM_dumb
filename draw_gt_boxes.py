...
def main():
    parser = argparse.ArgumentParser(description='Draw bounding boxes from gt.txt and generate videos')
    ...
    parser.add_argument('--output_name', type=str, default=None,
                       help='Output video filename (default: auto-generated from gt.txt path)')
    ...
    if seq_id is None:
        print(f"Error: Could not determine sequence ID from {args.gt_file}")
        return
    ...
    if args.output_name:
        output_filename = args.output_name if args.output_name.endswith('.mp4') else f"{args.output_name}.mp4"
    else:
